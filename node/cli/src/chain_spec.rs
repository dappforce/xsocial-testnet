...
use xsocial_runtime::{BabeConfig, BalancesConfig, Block, CollatorSelectionConfig, EXISTENTIAL_DEPOSIT, GrandpaConfig, SessionConfig, SudoConfig, SystemConfig};
pub use xsocial_runtime::GenesisConfig;
use xsocial_runtime::opaque::SessionKeys;
...
pub fn testnet_genesis(
    ...
    const ENDOWMENT: Balance = 10_000_000 * EXISTENTIAL_DEPOSIT;
    const STASH: Balance = ENDOWMENT / 1000;

    GenesisConfig {
        system: SystemConfig {
            code: xsocial_runtime::WASM_BINARY
                .expect("WASM binary was not build, please build it!")
                .to_vec(),
        },
        collator_selection: CollatorSelectionConfig {
            invulnerables: initial_authorities.iter().cloned().map(|(_, acc, _, _)| acc).collect(),
        ...
        babe: BabeConfig {
            authorities: vec![],
            epoch_config: Some(xsocial_runtime::BABE_GENESIS_EPOCH_CONFIG),
        },
